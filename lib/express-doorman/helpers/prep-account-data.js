 ...
 * @param {Object} accountData - The prepared Doorman Account data that is now
 *  ready to be created.
 */

/**
 * Takes in form data from a registration request, and converts it into a
 * valid Doorman account object.
 *
 ...
 * @param {Object} doormanConfig - The Doorman configuration object.
 * @param {prepAccountDataCallback} callback - The callback to run.
 */
module.exports = function (formData, doormanConfig, callback) {
  if (!formData) {
    throw new Error('prepAccountData must be provided with a formData argument.');
  }

  if (!doormanConfig) {
    throw new Error('prepAccountData must be provided with a doormanConfig argument.');
  }
